...
$pdf->Cell(50,6,'NILAI PROMETHEE',1,0);
$pdf->Cell(30,6,'KETERANGAN',1,1);

$pdf->SetFont('Arial','',10);

$no  = 1;
$sql = $koneksi->query("SELECT tb_siswa.`nisn`, tb_siswa.`nama`, tb_nilai.* FROM tb_siswa JOIN tb_nilai ON tb_siswa.`id` = tb_nilai.`id_siswa` WHERE id_tahun_ajaran = $id_tahun_ajaran  AND keterangan_pr = 'Lulus' ORDER BY hasil_promethee DESC");
while ($row = mysqli_fetch_array($sql)){
    $pdf->Cell(12,6,$no++,1,0);
    $pdf->Cell(25,6,$row['nisn'],1,0);
    $pdf->Cell(73,6,$row['nama'],1,0);
    $pdf->Cell(50,6,$row['hasil_promethee'],1,0);
    $pdf->Cell(30,6,$row['keterangan_pr'],1,1); 
}
...
$pdf->Output('','Laporan Hasil Kelulusan Seleksi Siswa Kelas Unggulan dengan Promethee.pdf', false);
?>