...
$no  = 1;
$sql = $koneksi->query("SELECT tb_siswa.`nisn`, tb_siswa.`nama`, tb_nilai.* FROM tb_siswa JOIN tb_nilai ON tb_siswa.`id` = tb_nilai.`id_siswa` WHERE tb_siswa.`id_tahun_ajaran` = $id_tahun_ajaran ORDER BY tb_nilai.`hasil_profile_matching` DESC");
...
$pdf->Output('','Laporan Hasil Seleksi Siswa Kelas Unggulan dengan Profile Matching.pdf', false);
?>